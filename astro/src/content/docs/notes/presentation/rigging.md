...
[Here](/notes/presentation/rigging#additional-helpfull-videos) are also some relevant videos, but I recommend looking through them after you've finished reading the rigging page.
...
Before we get into [the workflow](/notes/presentation/rigging#the-workflow), let's take a look at how weights work in general and how Blender handles them.
...
Then instead of following the normal [binding](/notes/presentation/rigging#binding-mesh-to-rig) process, which would mess with the weight painting in the vertex groups, one does it manually. First select all the character's objects and parent them to the rig *(Ctrl + P)*. Then all the objects need an armature modifier which points to the rig. To do this, one first has to make sure that the built-in [Copy Attribute Menu](https://docs.blender.org/manual/en/latest/addons/interface/copy_attributes.html) add-on is enabled. Then one simply sets up one armature modifier on any of the objects, selects the rest of the objects and in the `Copy Attributes` menu *(Ctrl + C)* selects `Copy Selected Modifiers`.
...
If you haven't read through the [what makes game readyrigs unique](/notes/presentation/rigging#what-makes-game-ready-rigs-unique) section, then do that first.
